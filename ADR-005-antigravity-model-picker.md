...
5. If the exact combined row is not directly visible at the top level, but the requested model is visible with a different effort tier that supports a flyout submenu, reveal the submenu by momentarily hovering the model row through the trusted action boundary, deliver `AXPress` to the exact matching effort item (`Low`, `Medium`, or `High`), and immediately restore the mouse pointer.
