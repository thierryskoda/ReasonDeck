...
4. Resolve the requested row by an exact, source-owned full-title mapping. Antigravity's exact `Fast`-badged Flash rows map to the canonical selection across tiers; substring, prefix, model-only, and unknown trailing-copy matches remain invalid. A missing or duplicate exact row changes nothing.
5. If the exact combined row is not directly visible at the top level, but the requested model is visible with a different effort tier that supports a flyout submenu, reveal the submenu by momentarily hovering the model row through the trusted action boundary, navigate into the verified submenu (Right Arrow, stepping Down Arrow to the verified item, and Return) through the trusted action boundary, and immediately restore the mouse pointer.
6. Deliver `AXPress` through the trusted action boundary and require the verified picker to close.
7. Reopen a fresh picker and parse its exact current-selection title back into the closed typed selection. Report success only when both observed values equal the request.
8. After exact read-back, accept an already-closed picker as the desired terminal UI state. Send Escape only while a fresh snapshot still proves the same Antigravity picker is open.
9. Read titles only from allowlisted control roles and retain only recognized closed labels. Never inspect editor or transcript text-node values, log unknown Accessibility text, or add network access.
