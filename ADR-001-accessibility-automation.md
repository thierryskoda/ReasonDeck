...
6. Verify the observed native picker title after each phase. The phase returns that verified title to transaction orchestration, which must reuse it instead of reopening the picker for the same observation. Skip phases that already match. If effort fails after model succeeds, report a partial result without rollback.
