...
final class MenuBarViewModel {
    ...
    func apply(_ id: UUID) {
        guard let invocation = currentInvocation(for: id) else {
            status = .failure("Bring ChatGPT, Claude Code, or Cursor to the front with an active window.")
            NSSound.beep()
            return
        }
        apply(invocation)
...
struct MenuBarContent: View {
    ...
    var body: some View {
        if model.readiness.installLocation != .installed {
            ...
            Text("ReasonDeck needs Accessibility permission to select model-menu controls. It only inspects the active ChatGPT, Claude Code, or Cursor window.")
